...
object Exercises {
    ...
    def max1(xs: Seq[Int]): Int = {
        if(xs.nonEmpty) return xs.max
        throw new NoSuchElementException
    }

    def max2(xs: Seq[Int]): Seq[Int] = {
        if(xs.nonEmpty) return Seq(xs.max)
        Seq()
    }

    def max3(xs: Seq[Int]): Option[Int] = {
        if(xs.nonEmpty) return Some(xs.max)
        None
    }
    ...
    sealed class MyIterable[Int] (x: Int, y: Int) extends Iterable[Int] {
        override def iterator: Iterator[Int] = Iterator(y, x)
    }
    ...
    def sum3(x: Int, y: Int): Int = sumIntegers(new MyIterable[Int](x,y))
