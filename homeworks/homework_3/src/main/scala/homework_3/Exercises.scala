...
object Exercises {
    ...
    def prettyBooleanFormatter2(x: Any): String = if (x.isInstanceOf[Boolean]) {
        formatBoolean(x.asInstanceOf[Boolean])
    } else {
        x.toString
    }

        def prettyBooleanFormatter3(x: Any): String = if (x.getClass.getName.contains("Boolean")) {
            formatBoolean(x.asInstanceOf[Boolean])
        } else {
            x.toString
        }

    private def formatBoolean(x: Boolean): String = if (x) "правда" else "ложь"
    ...
    def sum2(x: Int, y: Int): Int = sumIntegers(Pair(x, y))
    def sum3(x: Int, y: Int): Int = sumIntegers(List(x, y))

    case class Pair(first: Int, second: Int) extends Iterable[Int] {
        override def iterator(): Iterator[Int] = Iterator(first, second)
    }
