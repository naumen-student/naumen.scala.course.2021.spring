object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        var primeValues: Seq[Int] = Seq()
        var primeDividers: Seq[Int] = Seq()
        for {i <- 2 to number} {
            ...
            if (isPrime) {
                primeValues = primeValues :+ i
                if (number % i == 0) {
                    primeDividers = primeDividers :+ i
                }
            }
        }
        primeDividers
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, func: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D): Double = func(leftVec0, leftVec1) + func(rightVec0, rightVec1)
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.sortBy(ball => (4/3) * Math.PI * Math.pow(ball._2._1, 3) * ball._2._2).map(ball => ball._1)
    }
