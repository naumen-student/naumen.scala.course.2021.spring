...
object Exercises {
    ...
    def max2(xs: Seq[Int]): Seq[Int] = {
        if (xs.isEmpty) Seq[Int]() else Seq[Int](xs.max)
    }
    ...
    case class CustomCollection(first: Int, second: Int) extends Iterable[Int] {
    ...
    def sum2(x: Int, y: Int): Int = sumIntegers(ListSet(x, y))
    def sum3(x: Int, y: Int): Int = sumIntegers(CustomCollection(x, y))
}
