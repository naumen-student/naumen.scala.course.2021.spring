...
object Exercises {
    ...
    def divBy(iFrom: Int, iTo: Int, divs: Seq[Int]): Seq[Int] = {
        for {i <- iFrom to iTo
             if divs.exists(i % _ == 0)
             } yield i
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.sortWith((ball1, ball2) =>
            calculateMass(ball1._2._2, calculateSphereVolume(ball1._2._1)) <
              calculateMass(ball2._2._2, calculateSphereVolume(ball2._2._1))).map(ball => ball._1)
    }

    def calculateMass(density: Double, volume: Double): Double = density * volume

    def calculateSphereVolume(radius: Double): Double = 4 / 3 * Math.PI * Math.pow(radius, 3)
}