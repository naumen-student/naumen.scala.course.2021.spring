object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        val numbers = for {i <- iFrom to iTo
                    if i % 3 == 0 || i % 5 == 0
                    } yield  i
        numbers.sum
    }
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def process(number: Int, denominator: Int = 2, resultSet: Set[Int] = Set.empty[Int]): Set[Int] = {

            val denominatorQuad = denominator * denominator

            if (denominatorQuad<= number){
                if (number % denominator != 0) {
                    process(number, denominator+1, resultSet)
                }
                else {
                    process(number/denominator, denominator, resultSet+denominator)
                }
            }
            else {
                if (number > 1)
...
//        ballsArray.map(x => x._1 -> x._2._2 * 4.0 / 3.0 * java.lang.Math.PI * scala.math.pow(x._2._1, 3)).toSeq.sortBy(x => x._2).map(x => x._1)
        ballsArray.mapValues(x => x._2 * scala.math.pow(x._1, 3)).toSeq.sortBy(x => x._2).map(x => x._1)
        //Seq()
    }
