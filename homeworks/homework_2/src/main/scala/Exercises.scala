object Exercises extends App {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def factorize(num: Int, i: Int): Seq[Int] = {
            if (i * i > num){
                Seq(num)
            }else {
                if (num % i == 0) {
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, function: (Vector2D, Vector2D) => Double,
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.sortBy(x => ( 4 * java.lang.Math.PI * java.lang.Math.pow(x._2._1, 3) * x._2._2) / 3).map(_._1)
    }
}
