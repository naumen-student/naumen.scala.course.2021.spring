import Exercises.{Vector2D, balls}
import utest._

import scala.math.abs

object Test extends TestSuite{

    val tests = Tests{
        'test_divBy3Or7 - {
            assert(Exercises.divBy3Or7(1, 3) == Seq(3))
            assert(Exercises.divBy3Or7(5, 9) == Seq(6, 7, 9))
            assert(Exercises.divBy3Or7(0, 100) == Seq(0, 3, 6, 7, 9, 12, 14, 15, 18, 21, 24, 27, 28, 30, 33, 35, 36, 39, 42, 45, 48, 49, 51, 54, 56, 57, 60, 63, 66, 69, 70, 72, 75, 77, 78, 81, 84, 87, 90, 91, 93, 96, 98, 99))
        }
        'sumOfDivBy3Or5 - {
            assert(Exercises.sumOfDivBy3Or5(1, 3) == 3)
            assert(Exercises.sumOfDivBy3Or5(5, 9) == 20)
            assert(Exercises.sumOfDivBy3Or5(0, 10) == 33)
        }
        'primeFactor - {
            assert(Exercises.primeFactor(80) == Seq(2, 5))
            assert(Exercises.primeFactor(98) == Seq(2, 7))
            assert(Exercises.primeFactor(32732) == Seq(2, 7, 167))
            assert(Exercises.primeFactor(199) == Seq(199))
            assert(Exercises.primeFactor(169) == Seq(13))
        }
        'sumScalars - {
            assert(Exercises.sumScalars(Vector2D(5, -4), Vector2D(2, 1), Vector2D(0, 3), Vector2D(7, -1)) == 3)
            assert(Exercises.sumScalars(Vector2D(5, -4), Vector2D(2, 1), Vector2D(5, 2), Vector2D(4, -1)) == 24)
            assert(Exercises.sumScalars(Vector2D(5, 2), Vector2D(4, -1), Vector2D(0, 3), Vector2D(7, -1)) == 15)
        }
        'sumCosines - {
            assert(Exercises.sumCosines(Vector2D(0, 1), Vector2D(1, 0), Vector2D(-1, 0), Vector2D(0, -1)) == 0)
            assert(Exercises.sumCosines(Vector2D(0, 1), Vector2D(0, 2), Vector2D(-1, 0), Vector2D(-2, 0)) == 2)
            assert(abs(Exercises.sumCosines(Vector2D(1, 0), Vector2D(3, 4), Vector2D(4, 3), Vector2D(1, 0)) - 1.4) <= 0.1)
        }
        'sortByHeavyweight - {
            assert(Exercises.sortByHeavyweight(balls) ==
            Seq("Tin", "Platinum", "Nickel", "Aluminum", "Titanium", "Lead", "Sodium", "Uranium", "Gold", "Tungsten",
                "Zirconium", "Chrome", "Iron", "Copper", "Silver", "Plutonium", "Cobalt", "Cesium", "Calcium", "Lithium",
                "Magnesium", "Potassium", "Graphite"))
        }
    }
}
