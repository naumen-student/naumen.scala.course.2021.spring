...
object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        Range(iFrom, iTo + 1).filter(i => i % 3 == 0 || i % 5 == 0).sum
    }
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def isPrime(num: Int, i: Int = 2): Boolean = {
        ...
        ballsArray.map(
            {
                case (name, (radius, density)) =>
                    val volume = 4 * java.lang.Math.PI * java.lang.Math.pow(radius, 3) / 3
                    val weight = volume * density
                    name -> weight
            }
