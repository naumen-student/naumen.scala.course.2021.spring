object Exercises {
    ...
    def divBy3Or7(iFrom: Int, iTo: Int): Seq[Int] = divBy2Numbers(iFrom, iTo, 3, 7)
    ...
    на 3 или на 5.*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = divBy2Numbers(iFrom, iTo, 3, 5).sum

    def divBy2Numbers(iFrom: Int, iTo: Int, divBy1: Int, divBy2: Int): Seq[Int] = {
        for {i <- iFrom to iTo
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def primeFactor(number: Int, denominator: Int): Seq[Int] = {
            if (denominator * denominator > number)
                return Seq(number)

            if (number % denominator == 0)
                denominator +: primeFactor(number / denominator, denominator)
            else
                primeFactor(number, denominator + 1)
        }

        primeFactor(number, 2).distinct
    }
    ...
    def sumByFunc(leftVec0: Vector2D,
                  ...
                  rightVec0: Vector2D,
                  rightVec1: Vector2D) = func(leftVec0, leftVec1) + func(rightVec0, rightVec1)
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray.toSeq.sortBy(entry => getBallWeight(entry._2._1, entry._2._2)).map(_._1)

    def getBallWeight(radius: Int, density: Double): Double = density * (4 / 3) * Math.PI * Math.pow(radius, 3)
