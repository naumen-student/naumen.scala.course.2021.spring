object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    def factorize(number: Int, divider: Int): Seq[Int] =
      if (divider * divider > number) Seq(number)
      else number % divider match {
        case 0 => divider +: factorize(number / divider, divider)
        case _ => factorize(number, divider + 1)
      }
  ...
  */
  ...
  def getBallMass(radius: Int, density: Double): Double = 4 / 3 * Math.PI * Math.pow(radius, 3) * density

  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
    ballsArray
      .map(ball => (ball._1, getBallMass(ball._2._1, ball._2._2)))
      .toList
      .sortBy(_._2)
      .map(_._1)
}