object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] ={
        var res = Seq[Int]()
        var num = number
        for (i <- 2 to scala.math.sqrt(num).toInt){
            while (num % i == 0){
                num = num / i
                if (! res.contains(i))
                    res = i +: res
        ...
        if (num != 1 && ! res.contains(num)) res = res :+ num
        res.sorted
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray.transform((name, param) => scala.math.pow(param._1, 3) * (4f/3) * java.lang.Math.PI * param._2).toSeq.sortBy(_._2).map(_._1)
