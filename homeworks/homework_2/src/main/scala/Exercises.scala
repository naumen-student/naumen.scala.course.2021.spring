...
object Exercises {
  ...
  def divBy3Or7(iFrom: Int, iTo: Int): Seq[Int] = (iFrom to iTo).filter(e => e % 3 == 0 || e % 7 == 0)
  ...
  на 3 или на 5.*/
  /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
  def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (iFrom to iTo).filter(e => e % 3 == 0 || e % 5 == 0).sum
  ...
  def primeFactor(number: Int): Seq[Int] = {
    ...
    val put = (e: Int) => if (e > 1 && (result.isEmpty || result.last != e)) result += e
  ...
  def sumByFunc(
     leftVec0: Vector2D,
     leftVec1: Vector2D,
     flatten: (Vector2D, Vector2D) => Double,
     rightVec0: Vector2D,
     rightVec1: Vector2D,
   ): Double = flatten(leftVec0, leftVec1) + flatten(rightVec0, rightVec1)
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
    ballsArray.mapValues({ case (r, d) => (4 / 3) * Math.PI * Math.pow(r, 3) * d }).toSeq.sortBy(e => e._2).map(e => e._1)
}
