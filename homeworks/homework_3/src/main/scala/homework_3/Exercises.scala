...
object Exercises {
    ...
    def prettyBooleanFormatter2(x: Any): String = {
        val booleanClassname = "java.lang.Boolean"
        if (x.getClass.getName != booleanClassname)
            x.toString
        else if (x.asInstanceOf[Boolean])
            "правда"
        else
    ...
    def max2(xs: Seq[Int]): Seq[Int] = if (xs.isEmpty) Seq[Int]() else Seq(xs.max)
    ...
    def mergeNumbers(x: Int, y: Int): List[Int] = {
        return List(x, y)
    }

    case class TwoElementsSequence(x: Int, y: Int) extends Iterable[Int] {
        override def iterator: Iterator[Int] = Iterator(x, y)
    }

    def sum1(x: Int, y: Int): Int = sumIntegers(Seq(x, y))
    def sum2(x: Int, y: Int): Int = sumIntegers(mergeNumbers(x, y))
    def sum3(x: Int, y: Int): Int = sumIntegers(TwoElementsSequence(x, y))
}