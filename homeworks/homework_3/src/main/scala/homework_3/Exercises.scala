...
object Exercises {
  ...
  def max2(xs: Seq[Int]): Seq[Int] =
    xs.sorted.reverse.take(1)

  def max3(xs: Seq[Int]): Option[Int] =
    xs.sorted.reverse.headOption

  /** Задание №3
    * Допустим дана функция sumIntegers, которая умеет суммировать числа.
    */

  case class CrazyCollection(x: Int, y: Int) extends Iterable[Int] {
  ...
  def sum3(x: Int, y: Int): Int = sumIntegers(CrazyCollection(x, y))
}
