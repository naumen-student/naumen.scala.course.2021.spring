...
object Exercises {
  ...
  def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, fun: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D) =
    fun(leftVec0, leftVec1) + fun(rightVec0, rightVec1)
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
    ballsArray
      .map(x => (x._1, x._2._2 * ((4d / 3) * Math.PI * Math.pow(x._2._1, 3))))
      .toArray
