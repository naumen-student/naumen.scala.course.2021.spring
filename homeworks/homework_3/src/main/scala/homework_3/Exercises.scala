...
object Exercises {
    ...
    def prettyBooleanFormatter3(x: Any): String = x.getClass.getSimpleName match {
    ...
    def max2(xs: Seq[Int]): Seq[Int] = {
        if (xs.isEmpty) Seq.empty else Seq(xs.max)
    }
     ...
     */
    case class TwoIntegers(x: Int, y: Int) extends Iterable[Int] {
    ...
    def sum3(x: Int, y: Int): Int = sumIntegers(TwoIntegers(x, y))
