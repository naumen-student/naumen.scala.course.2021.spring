...
object Exercises {
    ...
    def prettyBooleanFormatter1(x: Any): String = {
        if (x.isInstanceOf[Boolean]) formatBoolean(x.asInstanceOf[Boolean])
        else x.toString
    }
    ...
    def prettyBooleanFormatter3(x: Any): String = {
        if (x.getClass == Class.forName("java.lang.Boolean")) formatBoolean(x.asInstanceOf[Boolean])
        else x.toString
    }

    def formatBoolean(x: Boolean): String = x match {
        case true => "правда"
        case _ => "ложь"
    }
    ...
    def max1(xs: Seq[Int]): Int = xs.length match {
        case 0 => throw new IllegalArgumentException("xs length was zero")
        case _ => xs.max
    }
