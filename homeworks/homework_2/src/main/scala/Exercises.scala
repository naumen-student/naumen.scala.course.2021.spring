...
object Exercises {
    private def divBy(iFrom: Int, iTo: Int, dividers: Seq[Int]): Seq[Int] = {
        Range(iFrom, iTo + 1)
          .filter(i => dividers.exists(divider => i % divider == 0))
    }
    /*ПРИМЕР*/
    ...
    def primeFactor(number: Int): Seq[Int] = {
        Range(2, number + 1)
          .filter(i => number % i == 0 && isPrimeNumber(i))
    }

    private def isPrimeNumber(number: Int): Boolean = {
        !Range(2, sqrt(number).toInt + 1)
          .exists(i => number % i == 0)
    }
    ...
    private def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D,
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val calculateHeavyweight = (radius: Int, density: Double) => density * 4/3 * PI * radius * radius * radius
        ballsArray
          .map(ball => (ball._1, calculateHeavyweight(ball._2._1, ball._2._2)))
          .toList
          .sortWith(_._2 < _._2)
          .map(ball => ball._1)
    }
