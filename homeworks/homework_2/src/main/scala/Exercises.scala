object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def factorize(number: Int, factor: Int): Seq[Int] = {
            if (factor * factor > number)
                Seq(number)
            else {
                number % factor match {
                    case 0 => factor +: factorize(number / factor, factor)
                    case _ => factorize(number, factor + 1)
                }
            }
        }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.map(ball => (ball._1, 4/3 * math.Pi * math.pow(ball._2._1, 3) * ball._2._2))
          .toSeq
          .sortBy(ball => ball._2)
          .map(ball => ball._1)
    }
