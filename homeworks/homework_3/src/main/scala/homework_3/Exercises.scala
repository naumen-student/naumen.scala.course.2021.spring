...
object Exercises {
     ...
     */
    def max1(xs: Seq[Int]): Int = if (xs.isEmpty) throw new IllegalArgumentException("seq is empty") else xs.max
    ...
    def sum3(x: Int, y: Int): Int = sumIntegers(MyPair(x, y))

    case class MyPair(x:Int,y:Int) extends Iterable[Int]{
