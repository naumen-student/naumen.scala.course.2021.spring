object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        (iFrom to iTo).filter(elem => elem % 3 == 0 || elem % 5 == 0).sum
    }
    ...
    */
    ...
    def weightBy(radius: Int, density: Double): Double = Math.PI * (4 / 3) * density * Math.pow(radius, 3)
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray.toSeq
          .sortBy(element => weightBy(element._2._1, element._2._2))
