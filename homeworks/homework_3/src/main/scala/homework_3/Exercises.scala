...
object Exercises {
    ...
    def sum2(x: Int, y: Int): Int = sumIntegers(List(y,x))

    class MyClass[A](x: A, y: A) extends Iterable[A] {
      override def iterator: Iterator[A] = Iterator(x, y)
    }

    def sum3(x: Int, y: Int): Int = sumIntegers(new MyClass[Int](x,y))
}
