object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def isSimple(number: Int): Boolean = {
        ...
        for {i <- 2 until  number
             if number % i == 0 && isSimple(i)} yield i
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        def calcV(radius: Int): Double = (Math.pow(radius, 3) * Math.PI * 4.0) / 3.0
        def calcM(r: Int, p: Double) = calcV(r) * p
        ballsArray.map(ball => (ball._1, calcM(ball._2._1, ball._2._2))).toSeq.sortBy(_._2).map(_._1)
    }
