...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        val sequence: List[Int] = List.empty
        val counter: Int  = 2
        findChain(number, counter, sequence).distinct
    }

    @scala.annotation.tailrec
    def findChain(number: Int, counter: Int, sequence: Seq[Int]): Seq[Int] = {
        if(counter <= number){
            if(number % counter == 0){
                findChain(number / counter, counter, sequence :+ counter)
            }
            else{
                findChain(number, counter + 1, sequence)
            }
        }
        else sequence
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.map(ball => (ball._1, ball_cm3(ball._2._1, ball._2._2)))
          .toList.sortBy(ball => ball._2).map(ball => ball._1)
    }

    def ball_cm3(radius: Int, density:Double) : Double = {
