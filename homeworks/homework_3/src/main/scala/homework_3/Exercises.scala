...
object Exercises {
     ...
     */
    def prettyBooleanFormatter1(x: Any): String = if (x.isInstanceOf[Boolean]) {
        x match {
            case true => "правда"
            case false => "ложь"
        }
    ...
    def prettyBooleanFormatter2(x: Any): String = if (x.getClass.getSimpleName == "Boolean") {
        x match {
            case true => "правда"
            case false => "ложь"
        }
    ...
    def sum3(x: Int, y: Int): Int = sumIntegers(SeqOfTwo(x, y))

    case class SeqOfTwo(x: Int, y: Int) extends Iterable[Int] {
