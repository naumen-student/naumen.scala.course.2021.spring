...
object Exercises {
     ...
     */
    def getStringFromBoolean(boolean: Boolean): String = {
        if (boolean) "правда" else "ложь"
    }

    def prettyBooleanFormatter1(x: Any): String = {
        if (x.isInstanceOf[Boolean]) {
            return getStringFromBoolean(x.asInstanceOf[Boolean])
        }
        x.toString
    }
    ...
    def sum3(x: Int, y: Int): Int = sumIntegers(IterablePoint(x, y))
