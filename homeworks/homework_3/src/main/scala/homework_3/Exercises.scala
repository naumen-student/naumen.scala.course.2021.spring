...
object Exercises {
    ...
    val handleBoolean = (x: Boolean) => if (x) "правда" else "ложь"

    def prettyBooleanFormatter1(x: Any): String = x match {
        case bool: Boolean => handleBoolean(bool)
        case _: Any => x.toString
    }

    def prettyBooleanFormatter2(x: Any): String = {
        if (x.isInstanceOf[Boolean])
            handleBoolean(x.asInstanceOf[Boolean])
        else
            x.toString
    }
    ...
    class Container(x: Int, y: Int) extends Iterable[Int] {
        var i = 0
    ...
    class IterableTwoInt(x: Int, y: Int) extends Iterable[Int] {
        override def iterator: Iterator[Int] = Iterator(x, y)
    }

    def sum1(x: Int, y: Int): Int = sumIntegers(Seq(x, y))
    def sum2(x: Int, y: Int): Int = sumIntegers(new IterableTwoInt(x, y))
    def sum3(x: Int, y: Int): Int = sumIntegers(new Container(x, y))
