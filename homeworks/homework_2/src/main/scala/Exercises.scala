object Exercises {
  ...
  def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D,
                sumFunc: (Vector2D, Vector2D) => Double,
                rightVec0: Vector2D, rightVec1: Vector2D): Double =
                sumFunc(leftVec0, leftVec1) + sumFunc(rightVec0, rightVec1)
  ...
  def getBallMass(radius: Int, density: Double): Double = (4/3 * Math.PI * radius * radius) * density

  def sortByHeavyweight(
      ballsArray: Map[String, (Int, Double)] = balls
  ): Seq[String] = ballsArray
    .mapValues{ case (rad, den) => getBallMass(rad, den) }
    .toList
    .sortBy(_._2)
    .map(_._1)
}
