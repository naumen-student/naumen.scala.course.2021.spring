...
object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long =
        List.range(iFrom, iTo + 1).filter(x => x % 3 == 0 || x % 5 == 0).sum
    ...
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def getDelimiters(number: Int): Seq[Int] =
        List.range(1, math.abs(number) + 1).filter(x => number % x == 0)

    def primeFactor(number: Int): Seq[Int] =
        getDelimiters(number).filter(x => getDelimiters(x).length == 2)
    ...
    def sumByFunc(leftVec0: Vector2D,
                  leftVec1: Vector2D,
                  valueFunc: (Vector2D, Vector2D) => Double,
                  rightVec0: Vector2D,
                  rightVec1: Vector2D): Double =
        valueFunc(leftVec0, leftVec1) + valueFunc(rightVec0, rightVec1)
    ...
    */
    ...
    def calcWeight(radius: Int, density: Double): Double = 4.0 / 3.0 * math.Pi * math.pow(radius, 3) * density
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray.toSeq.sortBy(x=> calcWeight(x._2._1, x._2._2)).map(_._1)
