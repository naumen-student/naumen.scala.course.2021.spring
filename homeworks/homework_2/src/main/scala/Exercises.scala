...
object Exercises {
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ...
        val weights = scala.collection.mutable.Map[String, Double]()
        for (ball <- ballsArray) {
            weights.put(ball._1, 4 / 3 * pi * math.pow(ball._2._1, 3) * ball._2._2)
        }
        weights.toSeq.sortBy(ball => ball._2).map(ball => ball._1)
    }
