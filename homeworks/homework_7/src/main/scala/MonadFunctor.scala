...
trait Monad[F[_]] {
    ...
    def sequence[A](fas: List[F[A]]): F[List[A]] = fas.foldLeft(pure(List[A]()))((list, current) => map2(list, current)((a, b) => a :+ b))
...
object Functor {
    def functorFromMonad[F[_]](M: Monad[F]): Functor[F] = new Functor[F] {
        def map[A, B](a: F[A])(f: A => B): F[B] = M.flatMap(a)(a => M.pure(f(a)))
    }
}
