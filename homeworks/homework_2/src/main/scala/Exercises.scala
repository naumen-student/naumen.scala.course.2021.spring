object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    ...
    while (currentNumber != 1) {
      if (currentNumber % probe == 0) {
        currentNumber /= probe
        resultList = resultList :+ probe
      } else
        probe += 1
    }
    resultList.distinct.sorted
  }
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    ballsArray.map(x => (x._1, (4 / 3 * java.lang.Math.PI * Math.pow(x._2._1,3)) * x._2._2)).toSeq.sortBy(_._2).map(_._1)
  }
