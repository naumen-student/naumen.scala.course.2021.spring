...
object Exercises {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    @tailrec
    def primeFactorHelper(start: Int, current: Int, acc: Seq[Int]): Seq[Int] =
      if (current % start == 0) {
        primeFactorHelper(start, current / start, acc :+ start)
      } else if (start > current) {
        acc
      } else {
        primeFactorHelper(start + 1, current, acc)
      }

    primeFactorHelper(2, number, Nil).distinct
  }
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
    ballsArray.toSeq.map { case (name, (radius, density)) =>
      (name, 4 / 3 * math.Pi * radius * radius * radius * density)
    }.sortBy(_._2).map(_._1)
}

object app extends App {
  println(Exercises.sortByHeavyweight(Exercises.balls).mkString("\"", "\", \"", "\""))
}
