object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        def loop(number: Int, acc: Int): Seq[Int] = number match {
            case x if x <= 1 => Seq[Int]()
            case x if x % acc == 0 => acc +: loop(number / acc, acc)
            case _ => loop(number, acc + 1)
        }
        loop(number, 2).distinct
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        balls.map(x=>(x._1, x._2._2 * 4/3 * Math.PI * Math.pow(x._2._1,3)))
          .toSeq
          .sortBy(x=>x._2).map(x=>x._1)
    }
